...
SignalRJS.prototype.poll = function(req,res){
	...
	setTimeout(function(){
		var connection = self._connectionManager.getByToken(token);// Maybe its not gracefully disconnecting from the client 
		var transport = self._transports[connection.type];
		if(transport)
...
SignalRJS.prototype.hub = function(hubName,hubObject){
	...
	return this;// @note: it is returning back signalR 
};
